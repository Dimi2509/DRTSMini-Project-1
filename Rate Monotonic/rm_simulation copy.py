...
def visualize_gantt(compressed, timeline, pe_name="PE", hyperperiod=None, filename=""):
    """Create a Gantt chart visualization"""
    if not compressed:
        return
    
    # Get unique tasks (excluding IDLE)
    tasks = sorted(list(set([task for task, _ in compressed if task != "IDLE"])), key=lambda x: int(x))
    ...
    num_tasks = len(tasks) + 1  # +1 for IDLE row
    fig_height = max(8, num_tasks * 0.5)  # Dynamic height based on number of tasks
    ...
    for idx, task in enumerate(tasks):
        for start, duration in task_schedule[task]:
            ax.barh(idx, duration, left=start, height=0.85, 
                   color=color_map[task], edgecolor="black", linewidth=0.5)
        
        y_ticks.append(idx)
        y_labels.append(f"Task {task}")
    
    # Add IDLE as its own row
    idle_idx = len(tasks)
    t = 0
    for task, duration in compressed:
        if task == "IDLE":
            ax.barh(idle_idx, duration, left=t, height=0.85,
                   color=color_map["IDLE"], edgecolor="gray", linewidth=0.5)
        t += duration
    y_ticks.append(idle_idx)
    ...
    ax.set_title(f"Scheduling Timeline - {filename}", fontsize=14, fontweight='bold')
    ax.set_xlim(0, hyperperiod if hyperperiod else len(timeline))
    ax.set_ylim(-0.5, len(tasks) + 0.5)
    ax.invert_yaxis()  # Task 0 appears first at the top
    ...
    with warnings.catch_warnings():
        ...
        plt.tight_layout()
    plt.savefig(f'gantt_{filename}.png', dpi=150, bbox_inches='tight')
    print(f"\n✓ Gantt diagram saved as 'gantt_{filename}.png'")
    plt.close()

def visualize_gantt_single_timeline(compressed, timeline, pe_name="PE", hyperperiod=None, filename=""):
    """Create a single-timeline Gantt chart (all tasks on one line)"""
    if not compressed:
        return
    ...
    # Plot all tasks on a single timeline
    t = 0
    for task, duration in compressed:
        color = color_map[task]
        ax.barh(0, duration, left=t, height=0.8,
    ...
    ax.set_yticks([])
    ax.set_xlabel("Time (units)", fontsize=12)
    ax.set_title(f"Superimposed Timeline - {filename}", fontsize=14, fontweight='bold')
    ax.set_xlim(0, hyperperiod if hyperperiod else len(timeline))
    ax.set_ylim(-0.5, 0.5)
    ax.grid(axis='x', alpha=0.3)
    ...
    handles = [mpatches.Patch(facecolor=color_map[task], edgecolor='black', label=f'Task {task}')
               for task in tasks]
    handles.append(mpatches.Patch(facecolor=color_map["IDLE"], edgecolor='black', label='IDLE'))
    ax.legend(handles=handles, loc='upper center', bbox_to_anchor=(0.5, -0.15), ncol=10)
    
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", UserWarning)
        plt.tight_layout()
    plt.savefig(f'Gantt_single_{filename}.png', dpi=150, bbox_inches='tight')
    print(f"✓ Single-timeline Gantt diagram saved as 'Gantt_single_{filename}.png'")
    plt.close()

# --- Main Execution ---
filename = 'tasks_example3.csv' # Make sure this file exists
...
if all_tasks:
    ...
    for pe, tasks in tasks_by_pe.items():
        history, hyperperiod = simulate_pe(pe, tasks)
        compressed = print_gantt(history, pe_name=pe)
        visualize_gantt(compressed, history, pe_name=pe, hyperperiod=hyperperiod, filename=filename)
        visualize_gantt_single_timeline(compressed, history, pe_name=pe, hyperperiod=hyperperiod, filename=filename)
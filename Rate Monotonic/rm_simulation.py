...
def visualize_gantt(compressed, timeline, pe_name="PE", hyperperiod=None, filename=""):
    ...
    for task, duration in compressed:
        if task == "IDLE":
            ax.barh(idle_idx, duration, left=t, height=0.6,
...
def visualize_gantt_single_timeline(compressed, timeline, pe_name="PE", hyperperiod=None, filename=""):
    ...
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", UserWarning)
        plt.tight_layout()
    plt.savefig(f'gantt_single_{pe_name}.png', dpi=150, bbox_inches='tight')
    print(f"✓ Single-timeline Gantt diagram saved as 'gantt_single_{pe_name}.png'")
    plt.close()
